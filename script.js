...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('numRounds').addEventListener('change', e => {
        ...
        if (allScores.length > 0) {
            if (!confirm("Changing rounds resets current session. Continue?")) {
...
function switchView(viewId) {
    ...
    if (viewId === 'tournament') backToRankings(); // Reset tournament view on enter
}
...
function applyFormatUI(format) {
    ...
    badge.innerText = format.toUpperCase() + " MODE";
...
// UI Navigation for Tournament flow
...
function showCategorySelection(mode) {
    ...
    currentTournamentMode = mode; // 'single' or 'team'
...
function generateBracketForCategory(category) {
    ...
    if (currentTournamentMode === 'single') {
        ...
        // Group by Team within this Category
        const teamMap = {};
    ...
    // Seed matchups (1 v 8, 2 v 7, etc for visualization)
